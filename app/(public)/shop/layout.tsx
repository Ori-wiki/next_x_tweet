...
const DynamicShopMenu = dynamic(
  () => import('./ShopMenu').then((mod) => mod.ShopMenu),
  {
    ssr: false,
    loading: () => (
      <div className='mb-5 rounded-2xl border border-white/10 bg-white/3 px-4 py-3 text-sm text-white/60'>
    ...
    <div className='w-full space-y-5'>
      <div className='rounded-4xl border border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(56,189,248,0.22),transparent_45%),rgba(255,255,255,0.03)] p-6'>
        ...
        <p className='mt-3 max-w-2xl text-white/70'>
          The same product list is shown with SSR, SSG and ISR so you can
          compare data freshness and rendering strategy directly in the
          interface.
        </p>
      </div>
      <DynamicShopMenu />
